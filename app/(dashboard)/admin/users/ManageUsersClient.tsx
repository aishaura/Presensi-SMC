  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    if (res.error) {
      ...
      if (res.error) {
        ...
        // Ambil dari res.data (bukan res.user)
        if (res.data) {
                  ...
                  filteredUsers.map((u) => (
                    <tr key={u.id} className="hover:bg-gray-50/50">
                      ...
                      <td className="px-6 py-4 capitalize">
                        <span
                          className={`inline-flex px-2 py-0.5 rounded text-xs font-semibold ${u.role === 'admin' ? 'bg-indigo-50 text-indigo-700' : 'bg-slate-50 text-slate-700'
                            }`}
                        >
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold ${u.role === 'inactive'
                          ...
                          <div className="flex items-center justify-end gap-2">
                            ...
                            {u.role !== 'admin' && (
                              <button
                                ...
                                className={`p-1.5 rounded-lg transition disabled:opacity-50 ${u.role === 'inactive'
            ...
            <div className="bg-white rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl p-6 text-center space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                {alert.type === 'confirm' ? (
                  ...
                  <button
                    onClick={closeAlert}
                    className="w-full py-2 bg-gray-900 hover:bg-gray-800 text-white rounded-lg text-sm font-medium transition"
                  >
                    Tutup
                  </button>
                )}
